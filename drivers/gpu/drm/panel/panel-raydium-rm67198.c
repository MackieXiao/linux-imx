 ...
 */
static const struct cmd_set_entry mcs_rm67198[] = {
    {0xFE, 0x0E}, {0x4B, 0x00}, {0x4C, 0x0F}, {0x4D, 0x20},
    {0x4E, 0x40}, {0x4F, 0x60}, {0x50, 0xA0}, {0x51, 0xC0},
    {0x52, 0xE0}, {0x53, 0xFF}, {0xFE, 0x0D}, {0x42, 0x00},
    {0x18, 0x08}, {0x08, 0x41}, {0x46, 0x02}, {0x72, 0x09},
    {0x1E, 0x04}, {0x1E, 0x00}, {0xFE, 0x0A}, {0x24, 0x17},
    {0x04, 0x07}, {0x1A, 0x0C}, {0x0F, 0x44}, {0xFE, 0x0B},
    {0x28, 0x40}, {0x29, 0x4F}, {0xFE, 0x04}, {0x00, 0x0C},
    {0x05, 0x10}, {0x06, 0x08}, {0x08, 0x10}, {0x09, 0x08},
    {0x0A, 0xE6}, {0x0B, 0x8C}, {0x1A, 0x12}, /*{0x1E, 0xE0},*/
    {0x29, 0x93}, {0x2A, 0x93}, {0x2F, 0x02}, {0x31, 0x02},
    {0x33, 0x05}, {0x37, 0x2D}, {0x38, 0x2D}, {0x3A, 0x1E},
    {0x3B, 0x1E}, {0x3D, 0x27}, {0x3F, 0x80}, {0x40, 0x40},
    {0x41, 0xE0}, {0x4F, 0x2F}, {0x50, 0x1E}, {0xFE, 0x06},
    {0x00, 0xEC}, {0x05, 0x06}, {0x07, 0xA2}, {0x08, 0xEC},
    {0x0D, 0x04}, {0x0F, 0xA2}, {0x32, 0xEC}, {0x37, 0x07},
    {0x39, 0x83}, {0x3A, 0xEC}, {0x41, 0x06}, {0x43, 0x83},
    {0x44, 0xEC}, {0x49, 0x06}, {0x4B, 0xA2}, {0x4C, 0xEC},
    {0x51, 0x04}, {0x53, 0xA2}, {0x75, 0xEC}, {0x7A, 0x07},
    {0x7C, 0x83}, {0x7D, 0xEC}, {0x82, 0x06}, {0x84, 0x83},
    {0x85, 0xEC}, {0x86, 0x0F}, {0x87, 0xFF}, {0x88, 0x00},
    {0x8A, 0x02}, {0x8C, 0xA2}, {0x8D, 0xEE}, {0x8E, 0x01},
    {0x8F, 0xE8}, {0x90, 0x0A}, {0x92, 0x06}, {0x93, 0xA0},
    {0x94, 0xA8}, {0x95, 0xEC}, {0x96, 0x0F}, {0x97, 0xFF},
    {0x98, 0x00}, {0x9A, 0x02}, {0x9C, 0xA2}, {0xA6, 0x21},
    {0xA7, 0x05}, {0xA9, 0x06}, {0xAC, 0x04}, {0xB1, 0x12},
    {0xB2, 0x17}, {0xB3, 0x17}, {0xB4, 0x17}, {0xB5, 0x17},
    {0xB6, 0x11}, {0xB7, 0x08}, {0xB8, 0x09}, {0xB9, 0x06},
    {0xBA, 0x07}, {0xBB, 0x17}, {0xBC, 0x17}, {0xBD, 0x17},
    {0xBE, 0x17}, {0xBF, 0x17}, {0xC0, 0x17}, {0xC1, 0x17},
    {0xC2, 0x17}, {0xC3, 0x17}, {0xC4, 0x0F}, {0xC5, 0x0E},
    {0xC6, 0x00}, {0xC7, 0x01}, {0xC8, 0x10}, {0xFE, 0x00},
    {0xC2, 0x08}, {0x35, 0x00}, {0x51, 0x00},
};
...
static int rm67198_enable(struct rad_panel *panel)
{
    ...
    ret = rad_panel_push_cmd_list(dsi,
                      &mcs_rm67198[0],
                      ARRAY_SIZE(mcs_rm67198));
    if (ret < 0) {
        dev_err(dev, "Failed to send MCS (%d)\n", ret);
        goto fail;
    }

    ret = mipi_dsi_generic_write(dsi, (u8[]){ 0x44, 0x03, 0x80 }, 3);
    if (ret < 0)
        goto fail;

    ret = mipi_dsi_generic_write(dsi, (u8[]){ 0xB7, 0x50, 0x00 }, 3);
    if (ret < 0)
        goto fail;

    ret = mipi_dsi_generic_write(dsi, (u8[]){ 0x11, 0x00 }, 2);
    if (ret < 0)
        goto fail;
    ...
    ret = mipi_dsi_generic_write(dsi, (u8[]){ 0x35, 0x00 }, 2);
    if (ret < 0)
        goto fail;

    ret = mipi_dsi_generic_write(dsi, (u8[]){ 0x51, 0x60 }, 2);
    if (ret < 0)
        goto fail;
...
static const struct rad_platform_data rad_rm67198 = {
    .enable = &rm67198_enable,
};

static const struct of_device_id rad_of_match[] = {
    { .compatible = "raydium,rm67198", .data = &rad_rm67198 },
    { /* sentinel */ }
};
